    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,relative_humidity_2m,wind_speed_10m&daily=precipitation_probability_max&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=auto&forecast_days=1`;
          const res = await fetch(url);
          const data = await res.json();
          resolve({
            tempF: Math.round(data.current.temperature_2m),
            precipProbability: Math.round(data.daily.precipitation_probability_max?.[0] ?? 0),
